...
def load_json(zipfilename: str, path: str):

    with zipfile.ZipFile(zipfilename) as f:
        json_dict = json.loads(f.open(path, "r").read().decode())
    return json_dict


class Hybrid_zip(Dataset):
    ...
    def _get_db(self):
        ...
        for name in images[:4]:
            ...
            exist = any(name.replace("json", "png") == i for i in seg)
            if not exist:
                continue
            gt_db.append(name)
    ...
    def prepare_test_img(self, idx):
        ...
        img = load_img(self.img_zip, path)
    ...
    def get_ann_info(self, idx):
        ...
        for zipfilename, prefix in self.seg_prefix.items():
            ...
            seg_list.append(load_img(zipfilename, path, seg=True))
        json_list = []
        for zipfilename, prefix in self.det_prefix.items():
            path = osp.join(prefix, name.replace("jpg", "json"))
            json_list.append(load_json(zipfilename, path)["frames"][0]["objects"])
